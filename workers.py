...
WORKER_PERMISSION_PREAMBLE = (
    ...
    "\n"
    "  ask-raiken \"<question>\"\n"
    "     Ask Raiken Agent (his advisor surface) for guidance when you're "
    "stuck. BLOCKS until Raiken Agent returns; his advice comes back on "
    "stdout. Use this BEFORE giving up and kicking the task back to Rook — "
    "Raiken Agent will often see the unblock. You are asking for ADVICE; "
    "you still own the task. Depth-1 capped: if YOU are Raiken Agent, you "
    "cannot call this (the orchestrator will refuse).\n"
    "\n"
    "  compact-memory --file <name> --summary \"<one-line note>\"\n"
    "     Audit-log that you just wrote learnings to a memory file "
    "(docs/memory/) and cleared your own working context. Fire-and-forget. "
    "Call this AFTER you've actually edited the file; this only appends an "
    "entry to workers/memory_compaction_log.jsonl so Rook can trace memory "
    "changes if weirdness shows up later.\n"
    "\n"
